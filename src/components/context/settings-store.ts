...
import {
  customizerItems,
  type SettingPlugin,
} from "@/src/config/customizer-items";
import { customizerList } from "@/src/config/customizer-list";
import {
  customizerPlugins,
  type ValueId,
} from "@/src/config/customizer-plugins";

export const categoryIds = customizerList.map((item) => item.id);

const defaultCheckedComponents = {} as Record<string, boolean>;

const defaultCheckedPlugins = customizerList.reduce((acc: any, item: any) => {
  item.children.forEach((child: any) => {
    child.components?.forEach((component: any) => {
